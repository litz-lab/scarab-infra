...
def singularity_build_scarab_binary(user, scarab_path, scarab_build, singularity_home, container_prefix, githash, infra_dir, dbg_lvl=1, stream_build=False, nodelist=[]):
    ...
    try:
        ...
        build_cmd = [
                "singularity",
                    "exec",
                    f"--bind={CONTAINER_HOME},{SCARAB_HOME}",
...
def rebuild_scarab(infra_dir, scarab_path, user, container_home, container_prefix, githash, scarab_githash, scarab_build, container_manager = "docker", stream_build=False, dbg_lvl=1, nodelist=[]):
...
def finish_simulation(user, docker_home, descriptor_path, root_dir, experiment_name, image_tag_list, available_nodes, slurm_ids = None, dont_collect = False, container_manager = "docker"):
    ...
    if slurm_ids:
        ...
        sbatch_cmd = (
            f"sbatch -c 1 --mem 4G --dependency=afterany:{','.join(slurm_ids)} "
