...
def open_interactive_shell(user, descriptor_data, workloads_data, infra_dir, dbg_lvl = 1):
    ...
    try:
        ...
        # Ensure the experiment workspace exists so the shell can start there
        ...
        try:
            ...
            if is_container_running(docker_container_name, dbg_lvl):
                subprocess.run(["docker", "exec", "--privileged", "-it", f"--user={user}", f"--workdir={experiment_workdir}", docker_container_name, "/bin/bash"])
            else:
