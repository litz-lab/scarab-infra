...
def open_interactive_shell(user, descriptor_data, workloads_data, infra_dir, dbg_lvl = 1):
    ...
    try:
        ...
        if container_manager == "singularity":
            # Start shell
            SIMPOINT_MOUNT=f"\"{traces_dir}:/simpoint_traces:ro\""
            CONTAINER_HOME=f"\"{docker_home}:/home/{user}:rw\""
            # SCARAB_HOME=f"\"{docker_home}/simulations/{experiment_name}/scarab:/home/{user}/simulations/{experiment_name}/scarab:ro\""
            subprocess.run(["singularity", "shell", "--bind", f"{SIMPOINT_MOUNT},{CONTAINER_HOME}", \
                            "--env", f"user_id={local_uid}", \
                            "--env", f"group_id={local_gid}", \
                            "--env", f"username={user}", \
                            "--home", f"/home/{user}", \
                            "--env", f"APP_GROUPNAME={docker_prefix}", \
                            "--env", f"APPNAME={workload}", \
                            f"singularity_images/{docker_prefix}_{githash}.sif"])
