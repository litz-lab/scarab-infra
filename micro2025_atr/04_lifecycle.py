...
# === Stats ===
int_stats = [
    'MAP_STAGE_ONPATH_INT_REG_LIFECYCLE_SPEC_RELEASE_count',
    'MAP_STAGE_ONPATH_INT_REG_LIFECYCLE_NONSPEC_RELEASE_count',
    'MAP_STAGE_ONPATH_INT_REG_LIFECYCLE_TOTAL_count',
]
...
int_array = np.vstack([np.zeros_like(int_array[0]), int_array])
fp_array = np.vstack([np.zeros_like(fp_array[0]), fp_array])
int_array = np.diff(int_array, axis=0)
fp_array = np.diff(fp_array, axis=0)
...
labels = ["In-use", "Unused", "Verified-unused"]
...
for i in range(int_pct.shape[0]):
...
plt.savefig("fig/04_lifecycle.png", dpi=300)
plt.close()
