    ...
    try:
        ...
        for cluster_id, segment_id in cluster_map.items():
            ...
            if num_traces == 0:
                ...
                # end-of-execution in non-deterministic workloads).
                # Default: fail hard so the user notices. Opt-in recovery via
                # EMPTY_SEGMENT_ACTION=remove (set on the trace_configuration's
                # env_vars) drops the segment from opt.p.lpt0.99 / opt.w.lpt0.99
                # iff its weight is below EMPTY_TRACE_WEIGHT_THRESHOLD, with
                # .bak backups and weight renormalisation.
                action = os.environ.get("EMPTY_SEGMENT_ACTION", "fail").lower()
                if action == "remove":
