...
echo "fingerprint size: $lines, maxK: $maxK, dim: $dim"
# binary search with maxK
spCmd="$tmpdir/simpoint -maxK $maxK -dim $dim -fixedLength off -numInitSeeds 10 -loadFVFile $FPFILE -saveSimpoints $OUTDIR/simpoints/opt.p -saveSimpointWeights $OUTDIR/simpoints/opt.w -saveVectorWeights $OUTDIR/simpoints/vector.w -saveLabels $OUTDIR/simpoints/opt.l -coveragePct .99 &> $OUTDIR/simpoints/simp.opt.log"
