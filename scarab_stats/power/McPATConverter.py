...
if __name__ == "__main__":
    converter = McPATConverter()

    # === Generate initial JSON files from McPAT XML template ===
    converter.generate_json_from_template(
        xml_path="./xml/template.xml",
        output_dir="./test"
    )

    # === Update JSON with Scarab simulation PARAMS file ===
    converter.update_json_from_scarab_params(
        scarab_params_path="./PARAMS.out",
        mcpat_json_path="./test/params_table.json"
    )

    # === Update JSON with Scarab runtime power-related statistics ===
    converter.update_json_from_scarab_stats(
        scarab_stats_path="./power.pkl",
        mcpat_json_path="./test/stats_table.json"
    )

    # === Merge parameters and statistics to generate final McPAT XML ===
    converter.generate_xml_from_json(
        structure_path="./test/mcpat_structure.json",
        params_table_path="./test/params_table.json",
        stats_table_path="./test/stats_table.json",
        output_path="./xml/mcpat_infile.xml"
    )

    # === (Optional) Run McPAT with generated input ===
    # Example command to run in the shell:
