    ...
    def calculate_distribution_stats(self):
        ...
        for group in groups:
            ...
            if float(0) in list(count_sums) or float(0) in list(total_count_sums):
                ...
                group_calculations += [[stat, True, 0] + [np.nan] * len(count_sums) for stat in new_stats]
            ...
            group_calculations += [[f"group_{group}_total_mean", True, 0] + list(total_count_means), [f"group_{group}_mean", True, 0] + list(count_means),
                               [f"group_{group}_total_stddev", True, 0] + list(total_count_stddev), [f"group_{group}_stddev", True, 0] + list(count_stddev)]
            ...
            #                                        columns=stat_columns))
            group_calculations += [[f"{stat}_pct", True, 0] + list(total_count_percentages.loc[stat]) for stat in total_count_percentages.index]
            # for stat in total_count_percentages.index:
            ...
            # group_calculations.append(pd.DataFrame([[f"{stat}_pct", True, 0] + list(count_percentages.loc[stat]) for stat in count_percentages.index], 
                                                #    columns=stat_columns))
            group_calculations += [[f"{stat}_pct", True, 0] + list(count_percentages.loc[stat]) for stat in count_percentages.index]
            # for stat in count_percentages.index:
        ...
        cat_time = time.time()
        self.data = pd.concat([self.data, pd.DataFrame(group_calculations, columns=stat_columns)]).reset_index(drop=True)
