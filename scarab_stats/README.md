# How to use
To use the python scarab stats api, you first need to run an experiment using a descriptor JSON (found in the dcworkloads repo).

In the current fast-path workflow, you **first generate an experiment CSV** from that descriptor, then you load/plot from the CSV.
The descriptor JSON already encodes the root simulation directory and traces directory, so you no longer need to pass those paths
manually in Python.
...
Running the guide:
Run `pip3 install notebook` if notebook is not installed. If your distribution is adopting PEP 668 (Making Python base environments as "externally managed"), use venv. Then you can run `source serve_jupyter.sh` on bohr3 to start the server. It will take around 4 seconds to get a port, start the server, and generate the kill script.
...
Then create a `stat_aggregator` to generate and/or load an experiment CSV
`aggregator = scarab_stats.stat_aggregator()`

First generate a CSV (fast NumPy path):
`aggregator.write_experiment_csv_numpy("allbench_home/exp2.json", "allbench_home/exp2.csv", postprocess=True, skip_incomplete=True, jobs=8)`

Then load the CSV:
`experiment = aggregator.load_experiment_csv("allbench_home/exp2.csv")`
...
#### write_experiment_csv_numpy
Arguments: (experiment_file: str, outfile: str, slurm: bool = False, postprocess: bool = False, skip_incomplete: bool = False, jobs: int = 8)

Generate the experiment CSV using the fast NumPy-based collector. This reads the experiment descriptor JSON, expands it into
simpoint tasks, parses stats, and writes the CSV. If `postprocess` is True, derived IPC and distribution statistics are appended
to the same outfile.

- experiment_file: the experiment descriptor JSON
- outfile: path to write the CSV
- postprocess: if True, append derived IPC + distribution stats
- skip_incomplete: if True, skip incomplete simpoints (warnings printed)
...
#### Derived stats and saving
The current `Experiment` class is a read-only view over a generated CSV. To create derived stats, either:
- regenerate the CSV with `postprocess=True`, or
- compute derived values in your analysis code using `experiment.retrieve_stats(...)` / `experiment.return_raw_data(...)`.
